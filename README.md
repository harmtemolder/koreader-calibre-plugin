...
2. A "Floating point numbers" column to store the **current percent read**, with "Format for numbers" set to `{:.0%}`.

3. A regular "Text" column to store the **location you last stopped reading at**.
4. A "Rating" column to store your **rating** of the book, as entered on the book's status page.
5. A "Long text" column to store your **review** of the book, as entered on the book's status page.
6. A regular "Text" column to store the **reading status** of the book, as entered on the book status page ("Finished", "Reading", "On hold").
7. A "Date" column to store the **date on which the book's status was last modified**. (This is probably the date on which you marked it as read.)
...
9. A "Long text" column to store the **raw contents of the metadata sidecar**, with "Interpret this column as" set to "Plain text".
...
- I work in PyCharm, which offers a remote debugging server. Follow [these steps](https://harmtemolder.com/calibre-development-in-pycharm/) to set that up. To enable that in this plugin, set `PYDEVD` to `True` in `__init__.py`.
